...
$query = DB::queryFirstRow('SELECT * from org_users where email_address=%s', $email);
$role = DB::queryFirstRow('SELECT * from org_roles where id=%s', $query['role_id']);
...
$redirect_link = "https://vfmplatform.com/care_org/verify.php?".http_build_query($verification);
...
if($link_type == "registration"){
    $msg_body = registration_template($fullname, $role['role_desc'], $redirect_link);
    $subject = "Account Registration Notification";
}
elseif($link_type == "password_reset"){
