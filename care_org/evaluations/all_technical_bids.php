...
?>
        ...
        <td class="text-bold text-lg" style="color:#FF8C00" colspan="5">Quotations Received</td>
            ...
            $check_tech_evaluation = DB::queryFirstRow('SELECT * from evaluations where user_id=%s AND tender_id=%s AND vendor_id=%s AND stage=%s', $current_user['user_id'], $tender_id, $quote['vendor_id'], 2);
            ?>
            <form action="evaluate-technical" method="POST" enctype="multipart/form-data"> 
            ...
            <?php 
            if(!$check_tech_evaluation){ //No Evaluation
            ?>
            <th style="width:15%" class="text-primary"><i>Pending Evaluation</i></th>
            <td>
            <button class="btn btn-primary text-bold btn-icon btn-sm" type="submit"> Evaluate Technical</button>
            </td>
            <?php } 
            elseif($check_tech_evaluation['status'] == 1){ //Saved as Draft
            ?>
            <th style="width:15%" class="text-primary"><i>Technical Evaluation Ongoing</i></th>
            <td>
            <button class="btn btn-primary text-bold btn-icon btn-sm" type="submit"> Continue Technical Evaluation</button>
            </td>
            <?php } 
