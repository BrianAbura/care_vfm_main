...
<title>Tenders Closed | Care Uganda</title>
<?php 
...
<div id="page-head">
<div id="page-title">
<h3 class="text-overflow text-uppercase">Closed Tenders</h3>
</div>
</div>
    ...
    <div class="row">
        ...
        ?>
            ...
            foreach($submitted as $all){
                ...
                $tender = DB::queryFirstRow('SELECT * from tenders where tender_id=%s AND submission_date<=%s order by date_created desc', $all['tender_id'], $curDate,);
            ...
            <td class="text-dark"><?php echo date_format(date_create($tender['submission_date']), 'd/m/Y')." ".date_format(date_create($tender['submission_date']), 'h:i A'); ?></td>
            <td class="text-danger">Closed</td>
            <td>
            <div class="btn-group">
            <a class="btn btn-sm btn-mint" href="<?php echo $tender['tender_id']."-closed"?>"> View Details</a>
            </div>
            </td>
            </tr>
                <?php 
