...
class Vendor{
        ...
        public function AddVendor($status, $request, $unspsc, $attachments){
            ...
            if(isset($query)){
                $response = array(
                    'Status'=>'Error',
                    'Message'=> 'The Vendor Registration Number or Name already exists.'
                );
            }
            else{ 
                if($status == 1){
...
if($token == "add_vendor"){  
    ...
    if(empty($vendor_logo) || empty($reg_cert) || empty($trade_license)){
        ...
        $logo = array('vendor_id' => $vendor_id,'description' => 'Logo','document_file' => $vendor_logo);
        $cer = array('vendor_id' => $vendor_id,'description' => 'Certificate of Registration','document_file' => $reg_cert);
...
elseif($token == "edit_vendor"){ 
    ...
    //Logo
    $logo_file = DB::queryFirstRow('SELECT * from vendor_attachments where vendor_id=%s AND description=%s', $vendorID, 'Logo');
    ...
    $logo = array('vendor_id' => $vendorID,'description' => 'Logo','document_file' => $vendor_logo, 'id' => $logo_file['id']);
    array_push($attachments, $logo);

    //RegCert
    $reg_cert_file = DB::queryFirstRow('SELECT * from vendor_attachments where vendor_id=%s AND description=%s', $vendorID, 'Certificate of Registration');
    if(empty($reg_cert)){
    ...
    $cer = array('vendor_id' => $vendorID,'description' => 'Certificate of Registration','document_file' => $reg_cert, 'id' => $reg_cert_file['id']);
    array_push($attachments, $cer);
