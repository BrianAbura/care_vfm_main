...
<div id="content-container">
    <div id="page-head">
        <div id="page-title">
        <h3 class="text-overflow text-uppercase">Completed Evaluations</h3>
        </div>
    </div>
...
<?php
$chk_published = DB::query('SELECT DISTINCT tender_id from published_notice where vendor_id=%s', get_vendor_id($current_user['user_id']));
if(!$chk_published){
?>
<div class="alert alert-mint col-md-7">
<strong> There are no completed evaluations that you have participated in. Click <a href="../tenders">here</a> to view tenders.</strong>
</div>
<?php }
else{
?>
...
<?php 
$cnt = 1;
foreach($chk_published as $all){
    $tender = DB::queryFirstRow('SELECT * from tenders where tender_id=%s', $all['tender_id']);
...
<td class="text-dark"><?php echo $category['name'];?></td>
<td class="text-dark"><?php echo $proc_method['method_name'];?></td>
<td class="text-success text-semibold">Published</td>
<td>
<div class="btn-group">
<a class="btn btn-sm btn-mint text-semibold" href="<?php echo $tender['tender_id']."-notice"?>"> View Notice</a>
</div>
</td>
</tr>
    <?php 
