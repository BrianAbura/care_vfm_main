...
<ol class="breadcrumb">
<li></li>
<li class="active text-lg">Active Vendors</li>
</ol>
</div>
...
<div class="table-responsive">
		<table id="active_vendors_table" class="table table-vcenter mar-top" width="100%">
			...
			<?php 
			$cnt = 1;
			$vendors = DB::query('SELECT * from vendors where vendor_status=%s order by id desc',3);
			...
			<td><button class="btn btn-success btn-sm text-bold">Active</button></td>
			<td><?php echo date_format(date_create($vendor['date_created']), 'd/m/Y h:i');?></td>
			<td><a class="btn btn-info btn-sm text-bold" href="<?php echo $vendor['vendor_id']."-view"; ?>">View</td>
				</tr>
				<?php 
				$cnt ++;
				} ?>
