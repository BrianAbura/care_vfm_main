...
$quotes = DB::query('SELECT DISTINCT vendor_id from evaluation_summary where tender_id=%s AND stage=%d AND decision=%s', $tender_id, 2, "Responsive");
...
?>
        ...
        <td class="text-bold text-lg" style="color:#FF8C00" colspan="5">Quotations Received</td>
            ...
            $check_fin_evaluation = DB::queryFirstRow('SELECT * from financial_evaluations where user_id=%s AND tender_id=%s AND vendor_id=%s', $current_user['user_id'], $tender_id, $quote['vendor_id']);
            ?>
            <form action="evaluate-financial" method="POST" enctype="multipart/form-data"> 
            ...
            <?php 
            if(!$check_fin_evaluation){ //No Evaluation
            ?>
            <th style="width:15%" class="text-primary"><i>Pending Evaluation</i></th>
            <td>
            <button class="btn btn-primary text-bold btn-icon btn-sm" type="submit"> Evaluate Financials</button>
            </td>
            <?php } 
            elseif($check_fin_evaluation['status'] == 1){ //Saved as Draft
            ?>
            <th style="width:15%" class="text-primary"><i>Financial Evaluation Ongoing</i></th>
            <td>
            <button class="btn btn-primary text-bold btn-icon btn-sm" type="submit"> Continue Financial Evaluation</button>
            </td>
            <?php } 
