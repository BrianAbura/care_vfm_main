    ...
    <div id="container" class="effect aside-float aside-bright mainnav-sm page-fixedbar">
        ...
        <div class="boxed">
            <div id="content-container">
                <div id="page-head">
                    <div id="page-title">
                        <h3 class="text-overflow text-uppercase">Shortlisted Tenders</h3>
                    </div>
                </div>
                ...
                <div id="page-content">
					...
					<ul class="nav nav-tabs">
						<li><span><a href="../tenders" style="color:white"><button class="btn btn-dark">Public Tenders</button></a></span></li>
						<li> &nbsp;&nbsp;&nbsp;</li>
                        <li><span><a href="" style="color:white"><button class="btn btn-info">Shortlisted Tenders</button></a></span></li>
					</ul>
					...
					<div class="tab-content">
						...
						<div class="row">
							...
							$vendor = DB::queryFirstRow('SELECT * from vendors where vendor_user_id=%s AND vendor_status=%s', $current_user['user_id'], 3); //Active Vendor Only
                            $shtlsts = DB::query('SELECT * from tender_shortlist where vendor_id=%s AND submission_date>=%s', $vendor['vendor_id'], $curDate);
							if(!$shtlsts){
							?>
							<div class="alert alert-mint col-md-7">
								<strong> You have not been shortlisted for any tenders. Please check again later.</strong>
							</div>
							<?php }
							else{
							?>
								...
								<tbody>
									<?php 
									$cnt = 1;
									foreach($shtlsts as $shtlst){
                                        $shlst_tender = DB::queryFirstRow('SELECT * from tenders where tender_id=%s and status=%s order by submission_date', $shtlst['tender_id'], 5);
                                        if(!$shlst_tender){
                                            continue;
                                        }
										$category = DB::queryFirstRow('SELECT * from procurement_categories where id=%s', $shlst_tender['category']);
									?>
								<tr>
								<td class="text-dark text-bold"><?php echo $cnt;?></td>
								<td class="text-dark text-bold"><?php echo $shlst_tender['tender_id'];?></td>
								<td class="text-lg text-info text-bold"><?php echo $shlst_tender['tender_title'];?></td>
								<td class="text-dark text-lg"><?php echo $category['name'];?></td>
								<td class="text-dark text-lg"><?php echo date_format(date_create($shlst_tender['submission_date']), 'd/m/Y')." ".date_format(date_create($shlst_tender['submission_date']), 'h:i A'); ?></td>
								<td class="text-center">
									<div class="btn-group">
									<a class="btn btn-md btn-mint" href="<?php echo $shlst_tender['tender_id']."-view"?>"> View Details</a>
									</div>
								</td>
								</tr>
									<?php 
